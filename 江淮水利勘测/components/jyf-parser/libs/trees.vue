      ...
      <view v-if="n.name == 'img'" :class="'_img ' + n.attrs.class" :style="n.attrs.style" :data-attrs="n.attrs" @tap.stop="imgtap">
        <rich-text
          ...
          :nodes="[
            {
              attrs: {
                src: loading && (ctrl[i] || 0) < 2 ? loading : lazyLoad && !ctrl[i] ? placeholder : ctrl[i] == 3 ? errorImg : n.attrs.src || '',
                alt: n.attrs.alt || '',
                width: n.attrs.width || '',
...
export default {
  ...
  mounted() {
    for (this.top = this.$parent; this.top.$options.name != 'jyf-parser'; this.top = this.top.$parent);
    this.init()
  },
  // #ifdef APP-PLUS
  beforeDestroy() {
  ...
  methods: {
            ...
            setTimeout(() => {
              this.observer.observe('._img', (res) => {
                if (res.intersectionRatio) {
                  for (var j = this.nodes.length; j--; ) if (this.nodes[j].name == 'img') this.$set(this.ctrl, j, 1)
    ...
    play(e) {
      ...
      if (contexts.length > 1 && this.top.autopause)
        for (var i = contexts.length; i--; ) if (contexts[i].id != e.currentTarget.dataset.id) contexts[i].pause()
    },
    imgtap(e) {
